### Chemokine expression in the CNS cells {#chemokinesexpress}

#### Cellular distribution of chemokine and chemokine receptors in the brain {#expr}

Chemokines and CKRs are expressed by healthy neurons, microglia, astrocytes, oligodendrocytes, and endothelial cells.
They are widely distributed in the CNS and involved in many physiological brain functions [@adler_2005].
Chemokines and receptors expressed in the CNS under physiological conditions are summarized in Table 1.
...
Thus, these may lead to co-localization of chemokine and its corresponding receptors within the same cells, raising the possibility of autocrine signaling and complex cross-talks between different types of cells.

##### CXCL12/CXCR4/ACKR3 {#cxcl12}

CXCL12, along with their receptors CXCR4 and ACKR3, is constitutively expressed and widely distributed in the CNS, both during neurodevelopment and adulthood. <!-- They are widely distributed across the CNS. -->
Moreover, these molecules are present in nearly all CNS cell types, including endothelial, glial, and notably neuronal cells (see: Table 1).
...
Likewise, as mentioned in previous chapter, recent evidence has begun to implicate ACKR3 in chemokine-opioid interactions, leading to behavioral manifestations (see: \@ref(behavior), @ikeda_2013, @meyrath_2020).
...
Therefore, it is tempting to speculate that depending on the cell type, ACKR3 acts either as an atypical, arrestin-coupled scavenger chemokine receptor or as a classical GPCR, manifesting as biased receptor (see Section: \@ref(biasedsignaling)) 
However, this issue warrants further clarification. 
Additionally, it was recently demonstrated that following CXCL12 treatment, ACKR3 impairs  astrocytic gap-junctional communication by inducing a Cx43 internalization in a `r knitr::asis_output("\U03B2")`-arrestin2-dependent manner [@fumagalli_2020], whereas CXCR4 leads to NF-K`r knitr::asis_output("\U03B2")` activation with consequent Tumour Necrosis Factor `r knitr::asis_output("\U03B1")` (TNF`r knitr::asis_output("\U03B1")`) and glutamate release from these cells [@bezzi_2001; @cal_2008; @cal_2010].
