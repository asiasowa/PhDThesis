...
Briefly, receptors of CXCL12 and CX3CL1 chemokines are located in the amygdala (see Sections: \@ref(CX3CL1signal) and \@ref(CXCL12signal)); however, their specific distribution is not thoroughly examined. 
Thus, the first aim of this dissertation was to investigate the cellular distribution of chosen chemokines and their receptors in the BLA and the CeA. 

Functionally, previous studies have demonstrated complex electrophysiological effects of CXCL12 and CX3CL1, which differ between, among others, brain regions or neuronal populations (see Sections: \@ref(CX3CL1inthebrain) and \@ref(CXCL12inthebrain)).

Moreover, at least some of those effects were mediated by glial cells.
Accordingly, this dissertation further aimed to dissect the complex mechanisms underlying bidirectional crosstalk between neurons and glial cells (e.i., astrocytes and microglia) regarding its regulation of membrane properties and synaptic transmission in the amygdala.<!---na razie ok, może uda się coś lepszego --> 
...
Precisely, to determine the distribution of chemokines (CXCL12/CX3CL1) or chemokine receptors (CXCR4 and CX3CR1) in relation to specific cell types (neurons, microglia, astrocytes) in the chosen amygdalar nuclei (BLA/CeA), their co-localization with different population markers (NeuN, GFAP, IBA)  was examined.
Coronal rat brain slices containing the BLA and the CeA were selected and immunohistochemically processed using specific primary antibodies against chosen cell markers and the chemokine or chemokine receptor of interest. <!-- dodać markery-->
...
Notably, to verify whether astrocytes or microglia are involved in observed effects, in subsets of experiments, recordings were made in the presence of either astrocytic toxin (e.i., fluorocitrate) or the inhibitor of microglial activity (e.i., minocycline).<!-- were additionally pre-applied.-->
...
############################################################################################################
###################CXCL12 2nM############################################################
BLA_df_2nM<-readRDS(".//BLA//2nM//membrane_table_BLA_cxcl12_2nM.rds")
BLA_df_2nM$Cell <- as.factor(BLA_df_2nM$Cell)
BLA_df_2nM$Condition <- as.factor(BLA_df_2nM$Condition)

bad_Cells <- c(15,9,10,1) #15 - input restistance > 250, 9,10 - Cm > 300, 1- RMS > -50
BLA_df_2nM <- BLA_df_2nM %>% filter(!Cell %in% bad_Cells) 

BLA_df_2nM <- BLA_df_2nM %>% mutate(Experiment = "CXCL12(2nM)") %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, everything())
BLA_df_2nM_inter <- BLA_df_2nM %>% filter(Type == "interneuron")
BLA_df_2nM <- BLA_df_2nM %>% filter(Type == "PC")


###################5nM############################################################
BLA_df_5nM<-readRDS(".//BLA//5nM//membrane_table_BLA_cxcl12_5nM.rds")
BLA_df_5nM$Cell <- as.factor(BLA_df_5nM$Cell)
BLA_df_5nM$Condition <- as.factor(BLA_df_5nM$Condition)
BLA_df_5nM <- BLA_df_5nM %>% mutate(Experiment = "CXCL12(5nM)") %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, everything())
bad_Cells <- c(4, 10) # Rm - 478 -> 268 ; 10 - 20d14002 - only 1 spike after application
BLA_df_5nM <- BLA_df_5nM %>% filter(!Cell %in% bad_Cells)

BLA_df_5nM_inter <- BLA_df_5nM %>% filter(Type == "interneuron")
BLA_df_5nM <- BLA_df_5nM %>% filter(Type == "PC")

################################CeA####################################################

# cxcl12 2nM
CeA_df_2nM<-readRDS(".//CeA//2nM//membrane_table_CeA_cxcl12_2nM.rds")
CeA_df_2nM <- CeA_df_2nM %>% mutate(Experiment = 'CXCL12(2nM)') %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, Type , everything())
bad_Cells <- c(17, 4, 8, 11) #  17 sEPSC - RS - przymyka się cały czas, 4 - RS - bardzo późno zaczęła spike'ować, 8 LF - dziwny IV. 11 - RS - dziwny IV i bardzo malo sEPSC, 
CeA_df_2nM <- CeA_df_2nM %>% filter(!Cell %in% bad_Cells)
# cxcl12 5nM

CeA_df_5nM<-readRDS(".//CeA//5nM//membrane_table_CeA_cxcl12_5nM.rds")
CeA_df_5nM <- CeA_df_5nM %>% mutate(Experiment = 'CXCL12(5nM)') %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, Type , everything())
bad_Cells <- c(2, 10, 3, 9, 1, 15, 7, 8, 23) # Adaptation ratio - NA :( , 3 - poszarpana po podaniu (sEPSC), 9 - LF- based on IV -sEPSC absent 1, 15, 7 -> RS based on IV. sEPSC, 8 - LF - Rm 600 -> 200, 21325036 - IV po podaniu
CeA_df_5nM <- CeA_df_5nM %>% filter(!Cell %in% bad_Cells)


## AMD 
CeA_amd5nM_df <-readRDS(".//CeA//AMD//blocking5nM//membrane_table_CeA_AMD_CXCL12_5nM.rds") # 
CeA_amd5nM_df <- CeA_amd5nM_df %>% mutate(Experiment = 'AMD + CXCL12(5nM)') %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, Type , everything()) %>% dplyr::rename(sag_ratio_140 = sag_ratio)
bad_Cells <- c(8)
CeA_amd5nM_df <- CeA_amd5nM_df %>% filter(!Cell %in% bad_Cells)

## AMD alone 
CeA_amda_df <-readRDS(".//CeA//AMD//alone//membrane_table_CeA_AMD_alone.rds") # 
CeA_amda_df <- CeA_amda_df %>% mutate(Experiment = 'AMD') %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, Type, everything()) %>% dplyr::rename(sag_ratio_140 = sag_ratio)
CeA_amda_df$Condition <- factor(CeA_amda_df$Condition, levels = c("baseline", "AMD"))

## fluorocitrate
CeA_df_fluoro <-readRDS(".//CeA//fluorocitrate//membrane_table_CeA_fluoro_cxcl12_5nM.rds") # 
CeA_df_fluoro <- CeA_df_fluoro %>% mutate(Experiment = "FC + CXCL12(5nM)") %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, Type, everything()) %>% dplyr::rename(sag_ratio_140 = sag_ratio)
...
CeA_amd5nM_sEPSC<- CeA_amd5nM_sEPSC_RS %>% bind_rows(CeA_amd5nM_sEPSC_LF) %>% dplyr::mutate(Experiment = 'AMD + CXCL12(5nM)') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
CeA_sEPSC_fluoro <-CeA_fluoro_sEPSC_LF %>% dplyr::mutate(Experiment = "FC + CXCL12(5nM)") %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
CeA_sEPSC_fluoro$Filename <- as.character(CeA_sEPSC_fluoro$Filename)
#CeA_fluoro_sEPSC<- CeA_fluoro_sEPSC_RS %>% bind_rows(CeA_fluoro_sEPSC_LF) %>% dplyr::mutate(Experiment = 'AMD + CXCL12(5nM)') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
CeA_amda_sEPSC<- CeA_amda_sEPSC_RS %>% bind_rows(CeA_amda_sEPSC_LF) %>% dplyr::mutate(Experiment = 'AMD') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
CeA_amda_sEPSC$Condition <- plyr::revalue(CeA_amda_sEPSC$Condition, c("b"="baseline", "f" = "AMD"))
CeA_amda_sEPSC$Condition <- factor(CeA_amda_sEPSC$Condition, levels = c("baseline", "AMD"))
...
CeA_sIPSC_amda<- CeA_sIPSC_amda_RS %>% bind_rows(CeA_sIPSC_amda_LF) %>% dplyr::mutate(Experiment = 'AMD') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
CeA_sIPSC_amda$Condition <- plyr::revalue(CeA_sIPSC_amda$Condition, c("b"="baseline", "f" = "AMD"))
CeA_sIPSC_amda$Filename <- as.character(CeA_sIPSC_amda$Filename)

#####

CeA_sIPSC_cxcl12_2nM_RS <- as.tibble(read.csv(".//synaptic_transmission//eventer//st_sIPSC_CeA_RS_cxcl12_2nM.csv"))
...
CeA_sIPSC_cxcl12_5nM_RS <- as.tibble(read.csv(".//synaptic_transmission//eventer//st_sIPSC_CeA_RS_cxcl12_5nM.csv"))
CeA_sIPSC_cxcl12_5nM_RS <- CeA_sIPSC_cxcl12_5nM_RS %>% mutate(Type ='RS') %>% dplyr::select(Filename, Cell, Condition, Type, everything())
CeA_sIPSC_cxcl12_5nM_LF <- as.tibble(read.csv(".//synaptic_transmission//eventer//st_sIPSC_CeA_LF_cxcl12_5nM.csv"))
...
CeA_sIPSC_amd2nM<- CeA_sIPSC_amd2nM_RS %>% bind_rows(CeA_sIPSC_amd2nM_LF) %>% dplyr::mutate(Experiment = 'AMD + CXCL12(2nM)') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
CeA_sIPSC_amd5nM<- CeA_sIPSC_amd5nM_RS %>% bind_rows(CeA_sIPSC_amd5nM_LF) %>% dplyr::mutate(Experiment = 'AMD + CXCL12(5nM)') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
CeA_sIPSC_fluoro_5nM <- CeA_sIPSC_fluoro_5nM_RS %>% bind_rows(CeA_sIPSC_fluoro_5nM_LF) %>% mutate(Experiment = "FC + CXCL12(5nM)") %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_df_final_12 <- BLA_df_2nM %>% bind_rows(BLA_df_5nM)
...
CeA_df_final_12 <- CeA_df_2nM %>% bind_rows(CeA_df_5nM) %>% bind_rows(CeA_amd5nM_df) %>% bind_rows(CeA_amda_df) %>% bind_rows(CeA_df_fluoro)
CeA_df_final_12$Experiment <- factor(CeA_df_final_12$Experiment, levels = c("CXCL12(2nM)", "CXCL12(5nM)", "AMD + CXCL12(5nM)", "AMD", "FC + CXCL12(5nM)"))
CeA_df_final_12$Condition <- factor(CeA_df_final_12$Condition, levels = c("baseline", "CXCL12", "AMD"))
...
CeA_sEPSC_final_12$Condition <- factor(CeA_sEPSC_final_12$Condition, levels = c("baseline", "CXCL12", "AMD"))
CeA_sEPSC_final_12$Experiment <- factor(CeA_sEPSC_final_12$Experiment, levels = c("CXCL12(2nM)", "CXCL12(5nM)", "AMD + CXCL12(5nM)", "AMD", "FC + CXCL12(5nM)"))
...
CeA_sIPSC_final_12$Experiment <- factor(CeA_sIPSC_final_12$Experiment, levels = c("CXCL12(2nM)", "CXCL12(5nM)", "AMD + CXCL12(2nM)", "AMD + CXCL12(5nM)", "AMD", "FC + CXCL12(5nM)"))
CeA_sIPSC_final_12$Condition <- factor(CeA_sIPSC_final_12$Condition, levels = c("baseline", "CXCL12", "AMD"))
...
CeA_mIPSC_final_12$Experiment <- factor(CeA_mIPSC_final_12$Experiment, levels = c("CXCL12(2nM)", "CXCL12(5nM)")) #, levels = c("CXCL12(2nM)", "CXCL12(5nM)", "AMD + CXCL12(2nM)", "AMD"))
#CeA_mIPSC_final_12$Condition <- factor(CeA_mIPSC_final_12$Condition, levels = c("baseline", "CXCL12", "AMD"))
...
BLA_df_inter_12 <- BLA_df_5nM_inter %>% bind_rows(BLA_df_2nM_inter)
...
BLA_cxcl12_5nM_sEPSC_inter <- BLA_cxcl12_5nM_sEPSC_inter %>% mutate(Experiment = 'CXCL12(2nM)') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
#knitr::opts_chunk$set(echo = FALSE, dev = 'svg', fig.width = 10, fig.height = 7)
#library(elektroasia)
library(xml2)
library(pzfx)
...
# loading files from frak2nM
CeA_df_frak2nM<-readRDS(".//CeA//frak2nM//membrane_table_CeA.rds")
CeA_df_frak2nM <- CeA_df_frak2nM %>% mutate(Experiment = 'frak2nM') %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, Type , everything())
bad_Cells <- c(11) # 11 - 19o22019, Capacitane in baseline 141, resistance 190. # but 4 has nice sEPSC :(((( # based on lack of sEPSC - 7, 12, 14, 15 ##### 24.11.2020 added again 7,12,14,15 # based on IV - very late - 19405029 (cell nr 4, and sEPSC cell nr 3, but sEPSC is ok)
CeA_df_frak2nM <- CeA_df_frak2nM %>% filter(!Cell %in% bad_Cells)

# frak5nM
CeA_df_frak5nM<-readRDS(".//CeA//frak5nM//membrane_table_CeA_frak5nM.rds")
CeA_df_frak5nM <- CeA_df_frak5nM %>% mutate(Experiment = 'frak5nM') %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, Type , everything())
bad_Cells <- c(6)  # 21128037 - IV
...
CeA_azd_df <- CeA_azd_df %>% mutate(Experiment = 'AZD + frak2nM') %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, Type , everything()) %>% dplyr::rename(sag_ratio_140 = sag_ratio)
...
CeA_mino_df <- CeA_mino_df %>% mutate(Experiment = 'mino + frak2nM') %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, Type , everything()) %>% dplyr::rename(sag_ratio_140 = sag_ratio)
bad_Cells <- c(9) # Capacitance = 184, much higher than others in baseline
...
bad_Cells <- c(1, 13) # amplitude from 17->23; RISETIME > 2
CeA_LF_sIPSC <- CeA_LF_sIPSC %>% filter(!Cell %in% bad_Cells)
CeA_sIPSC_frak2nM <- CeA_RS_sIPSC %>% dplyr::bind_rows(CeA_LF_sIPSC) %>% mutate(Experiment = 'frak2nM')
...
CeA_sIPSC_azd<- CeA_azd_RS_sIPSC  %>% dplyr::bind_rows(CeA_azd_LF_sIPSC) %>% mutate(Experiment = 'AZD + frak2nM')
...
CeA_sIPSC_mino <- CeA_mino_RS_sIPSC %>% dplyr::bind_rows(CeA_mino_LF_sIPSC) %>% mutate(Experiment = 'mino + frak2nM')
...
CeA_sEPSC_frak2nM <- CeA_RS_sEPSC %>% dplyr::bind_rows(CeA_LF_sEPSC) %>% mutate(Experiment = 'frak2nM')
...
CeA_sEPSC_frak5nM <- CeA_RS_sEPSC %>% dplyr::bind_rows(CeA_LF_sEPSC) %>% mutate(Experiment = 'frak5nM')
...
CeA_sEPSC_azd<- CeA_azd_RS_sEPSC  %>% dplyr::bind_rows(CeA_azd_LF_sEPSC) %>% mutate(Experiment = 'AZD + frak2nM')
...
CeA_sEPSC_mino<- CeA_mino_RS_sEPSC  %>% dplyr::bind_rows(CeA_mino_LF_sEPSC) %>% mutate(Experiment = 'mino + frak2nM')



####################loading files from mino alone CeA
# minocycline alone 
...
bad_Cells <- c(9, 8, 10, 12) #poszarpana i brzydka - sEPSC; IV - RS - brzydkie 20d08045, 20813003, 20729032
CeA_df_minoa <- CeA_df_minoa %>% filter(!Cell %in% bad_Cells)


#######mino alone synaptic transmission#######################################
...
####################loading files from azd alone CeA
####################loading files from mino alone
# azd alone 
CeA_df_azda<-readRDS(".//AZD_alone//CeA//membrane_table_CeA_azd_alone.rds")
CeA_df_azda <- CeA_df_azda %>% mutate(Experiment = 'AZD') %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, Type , everything()) %>% dplyr::rename(sag_ratio_140 = sag_ratio)
CeA_df_azda$RMS <- as.numeric(CeA_df_azda$RMS)
bad_Cells <- c(10, 15, 12, 13, 17, 5, 9) # Capacitance almost 0, (0.1, 0.15) ## wrong matlab scripts, check it - 3, 9 //// 10 - strasznie poszarpana w sEPSC ;;; 12, 13 - resistance ; 20d11024 - 17 - IV; 20813033 20819008 IV, 
CeA_df_azda <- CeA_df_azda %>% dplyr::filter(!Cell %in% bad_Cells)
CeA_df_azda$Condition = factor(CeA_df_azda$Condition, levels=c('baseline', 'AZD'))
CeA_df_azda <- CeA_df_azda %>% arrange(Experiment, Filename_all1, Cell, Condition)

#######azd alone synaptic transmission#######################################

sIPSC_CeA_RS_azd_alone <- read.csv(".//synaptic_transmission//eventer//st_sIPSC_CeA_RS_azda.csv")
CeA_azda_RS_sIPSC <- as.tibble(sIPSC_CeA_RS_azd_alone) %>% mutate(Type ='RS', Experiment = 'AZD') %>% dplyr::select(Filename, Cell, Condition, Type, everything())
CeA_azda_RS_sIPSC$Condition <- plyr::revalue(CeA_azda_RS_sIPSC$Condition, c("b"="baseline", "f" = "AZD"))

sIPSC_CeA_LF_azd_alone <- read.csv(".//synaptic_transmission//eventer//st_sIPSC_CeA_LF_azda.csv")
CeA_azda_LF_sIPSC <- as.tibble(sIPSC_CeA_LF_azd_alone) %>% mutate(Type ='LF', Experiment = 'AZD') %>% dplyr::select(Filename, Cell, Condition, Type, everything())
CeA_azda_LF_sIPSC$Condition <- plyr::revalue(CeA_azda_LF_sIPSC$Condition, c("b"="baseline", "f" = "AZD"))
bad_Cells <- c("2") # amplitude 13 -> 24
CeA_azda_LF_sIPSC <- CeA_azda_LF_sIPSC %>% filter(!Cell %in% bad_Cells)


CeA_sIPSC_azda<- CeA_azda_RS_sIPSC  %>% dplyr::bind_rows(CeA_azda_LF_sIPSC) %>% mutate(Experiment = 'AZD')
#CeA_sIPSC_azda$Condition <- plyr::revalue(CeA_sIPSC_azda$Condition, c("b"="baseline", "f" = "AZD"))
CeA_sIPSC_azda$Condition = factor(CeA_sIPSC_azda$Condition, levels=c('baseline', 'AZD'))
...
sEPSC_CeA_azda_RS <- sEPSC_CeA_azda_RS %>% mutate(Type ='RS', Experiment = 'AZD') %>% dplyr::select(Filename, Cell, Condition, Type, everything())
sEPSC_CeA_azda_RS$Condition <- plyr::revalue(sEPSC_CeA_azda_RS$Condition, c("b"="baseline", "f" = "AZD"))
...
sEPSC_CeA_azda_LF <- sEPSC_CeA_azda_LF %>% mutate(Type ='LF', Experiment = 'AZD') %>% dplyr::select(Filename, Cell, Condition, Type, everything())
sEPSC_CeA_azda_LF$Condition <- plyr::revalue(sEPSC_CeA_azda_LF$Condition, c("b"="baseline", "f" = "AZD"))
...
CeA_sEPSC_azda<- sEPSC_CeA_azda_RS  %>% dplyr::bind_rows(sEPSC_CeA_azda_LF) %>% mutate(Experiment = 'AZD')
CeA_sEPSC_azda$Condition = factor(CeA_sEPSC_azda$Condition, levels=c('baseline', 'AZD'))


##################CeA big tables with all conditions#############################
...
CeA_df_final$Experiment = factor(CeA_df_final$Experiment, levels=c('frak2nM','frak5nM', 'AZD + frak2nM', 'AZD', 'mino + frak2nM', 'minocycline'))
...
CeA_df_final$Condition <- factor(CeA_df_final$Condition, levels = c("baseline", "fractalkine", "minocycline", "AZD"))
...
CeA_df_sag_RS <- CeA_df_sag %>% filter( Type == 'RS')
CeA_df_sag_LF <- CeA_df_sag %>% filter( Type == 'LF')
################final synaptic transmission files of CeA######################################################################
...
CeA_sIPSC_final$Condition <- plyr::revalue(CeA_sIPSC_final$Condition, c("b"="baseline", "f" = "fractalkine"))
CeA_sIPSC_final$Condition <- factor(CeA_sIPSC_final$Condition, levels = c("baseline", "fractalkine", "AZD", "minocycline"))
CeA_sIPSC_final$Cell <- rep(1:(nrow(CeA_sIPSC_final)/2), each = 2)
CeA_sIPSC_final$Experiment <- as.factor(CeA_sIPSC_final$Experiment)
CeA_sIPSC_final$Experiment = factor(CeA_sIPSC_final$Experiment, levels=c('frak2nM','AZD + frak2nM','AZD','mino + frak2nM', 'minocycline'))
...
CeA_frak2nM_mIPSC <- CeA_frak2nM_mIPSC %>% dplyr::mutate(Type = "LF") %>% dplyr::mutate(Experiment = "frak2nM")
CeA_frak2nM_mIPSC <- CeA_frak2nM_mIPSC %>% dplyr::select(Experiment, Filename, Cell, Condition, Type, everything()) %>% as_tibble()
CeA_frak2nM_mIPSC$Condition <- plyr::revalue(CeA_frak2nM_mIPSC$Condition, c("b"="baseline", "f" = "fractalkine"))


CeA_sEPSC_final <- CeA_sEPSC_frak2nM %>%  dplyr::bind_rows(CeA_sEPSC_azd) %>% dplyr::bind_rows(CeA_sEPSC_mino) %>% dplyr::bind_rows(CeA_sEPSC_minoa) %>% dplyr::bind_rows(CeA_sEPSC_azda) %>%dplyr::select(Experiment, everything()) # dplyr::bind_rows(CeA_sEPSC_frak5nM) %>% 12.08.2021
CeA_sEPSC_final <- as.tibble(CeA_sEPSC_final)
CeA_sEPSC_final$Condition <- plyr::revalue(CeA_sEPSC_final$Condition, c("b"="baseline", "f" = "fractalkine"))
CeA_sEPSC_final$Condition <- factor(CeA_sEPSC_final$Condition, levels = c("baseline", "fractalkine", "AZD", "minocycline"))
CeA_sEPSC_final$Cell <- rep(1:(nrow(CeA_sEPSC_final)/2), each = 2)
CeA_sEPSC_final$Experiment = factor(CeA_sEPSC_final$Experiment, levels=c('frak2nM','frak5nM', 'AZD + frak2nM', 'AZD', 'mino + frak2nM', 'minocycline'))
...
bad_Cells <- c(1, 14, 8)  #filtering 3 and 14 because of capacitance and 2 because of only 2 spikes and very depolarized, 1 - Rm more than 200, duży access, 13 - tau; ealier: 1, 2, 3, 13, 14. WHY 8? 
BLA_frak2nM_df <- BLA_frak2nM_df %>% filter(!Cell %in% bad_Cells) 
BLA_frak2nM_df <- BLA_frak2nM_df %>% mutate(Experiment = "frak2nM") %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, everything())
...
BLA_frak2nM_df_c <- BLA_frak2nM_df %>% filter(Condition == 'fractalkine')
...
BLA_frak5nM_df <- BLA_frak5nM_df %>% mutate(Experiment = "frak5nM") %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, everything())
...
BLA_frak5nM_df_c <- BLA_frak5nM_df %>% filter(Condition == 'fractalkine')
...
BLA_azd_df <- BLA_azd_df %>% mutate(Experiment = "AZD + frak2nM")  %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, everything())
BLA_azd_df <- BLA_azd_df %>% filter(Type == "PC")
BLA_azd_df_b <- BLA_azd_df %>% filter(Condition == 'baseline')
BLA_azd_df_c <- BLA_azd_df %>% filter(Condition == 'fractalkine')
...
BLA_anti_df <- BLA_anti_df %>% mutate(Experiment = "antibody + frak2nM") %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, everything()) #%>% dplyr::rename(sag_ratio_140 = sag_ratio_80)
BLA_anti_df <- BLA_anti_df %>% filter(Type == "PC")
BLA_anti_df_b <- BLA_anti_df %>% filter(Condition == 'baseline')
BLA_anti_df_c <- BLA_anti_df %>% filter(Condition == 'fractalkine')
...
BLA_mino_df <- BLA_mino_df %>% mutate(Experiment = "mino + frak2nM")  %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, everything())
...
BLA_mino_df_c <- BLA_mino_df %>% filter(Condition == 'fractalkine')
...
BLA_picro_df <- BLA_picro_df %>% mutate(Experiment = "picro + frak2nM") %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, everything()) #%>% dplyr::rename(sag_ratio_140 = sag_ratio)
BLA_picro_df <- BLA_picro_df %>% filter(Type == "PC")
BLA_picro_df_b <- BLA_picro_df %>% filter(Condition == 'baseline')
BLA_picro_df_c <- BLA_picro_df %>% filter(Condition == 'fractalkine')


######################################### SYNAPTIC TRANSMISSION ###############################################
#########################sEPSC #############################################################################
BLA_frak2nM_sEPSC <- as.tibble(read.csv(".//synaptic_transmission//eventer//st_sEPSC_BLA_frak2nM.csv"))
BLA_frak2nM_sEPSC <- BLA_frak2nM_sEPSC %>% dplyr::mutate(Experiment = 'frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_frak5nM_sEPSC <- BLA_frak5nM_sEPSC %>% dplyr::mutate(Experiment = 'frak5nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_azd_sEPSC <- BLA_azd_sEPSC %>% mutate(Experiment = 'AZD + frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_anti_sEPSC <- BLA_anti_sEPSC %>% mutate(Experiment = 'antibody + frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_mino_sEPSC <- BLA_mino_sEPSC %>% mutate(Experiment = 'mino + frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_picro_sEPSC <- BLA_picro_sEPSC %>% mutate(Experiment = 'picro + frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_frak2nM_sIPSC <- BLA_frak2nM_sIPSC %>% mutate(Experiment = 'frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_frak5nM_sIPSC <- BLA_frak5nM_sIPSC %>%  mutate(Experiment = 'frak5nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_azd_sIPSC <- BLA_azd_sIPSC %>% mutate(Experiment = 'AZD + frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
bad_Cells <- c(6) # too high frequency
BLA_azd_sIPSC <- BLA_azd_sIPSC %>% filter(!Cell %in% bad_Cells)
...
BLA_azdfrak5nM_sIPSC <- BLA_azdfrak5nM_sIPSC %>%  mutate(Experiment = 'AZD + frak5nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_anti_sIPSC <- BLA_anti_sIPSC %>% mutate(Experiment = 'antibody + frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_mino_sIPSC <- BLA_mino_sIPSC %>% mutate(Experiment = 'mino + frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_minofrak5nM_sIPSC <- BLA_minofrak5nM_sIPSC %>% mutate(Experiment = 'mino + frak5nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
##############loading minocycline BLA transmission files############################
...
BLA_df_azda <- BLA_df_azda %>% mutate(Experiment = 'AZD') %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, everything()) #%>% dplyr::rename(sag_ratio_140 = sag_ratio)
...
BLA_df_azda$Condition <- factor(BLA_df_azda$Condition, levels = c("baseline", "AZD"))
BLA_df_azda <- BLA_df_azda %>% dplyr::arrange(Experiment, Filename_all1, Cell, Condition)
BLA_df_azda <- BLA_df_azda %>% filter(Type == "PC")
##############loading AZD BLA transmission files############################

BLA_azda_sEPSC <- as.tibble(read.csv(".//synaptic_transmission//eventer//st_sEPSC_BLA_azda.csv"))
BLA_azda_sEPSC <- BLA_azda_sEPSC %>% mutate(Experiment = 'AZD') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
#bad_Cells <- c(5, 3) #weird IV - 2082002
#BLA_azda_sEPSC <- BLA_azda_sEPSC %>% filter(!Cell %in% bad_Cells)
BLA_azda_sEPSC$Condition <- plyr::revalue(BLA_azda_sEPSC$Condition, c("b"="baseline", "f" = "AZD"))
BLA_azda_sEPSC$Condition <- factor(BLA_azda_sEPSC$Condition, levels = c("baseline", "AZD"))
...
BLA_azda_sIPSC <- BLA_azda_sIPSC %>% mutate(Experiment = 'AZD') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
BLA_azda_sIPSC$Condition <- plyr::revalue(BLA_azda_sIPSC$Condition, c("b"="baseline", "f" = "AZD"))
BLA_azda_sIPSC$Condition <- factor(BLA_azda_sIPSC$Condition, levels = c("baseline", "AZD"))
...
BLA_df_final$Experiment = factor(BLA_df_final$Experiment, levels=c('frak2nM', 'frak5nM','antibody + frak2nM','AZD + frak2nM','AZD', 'mino + frak2nM', 'minocycline', 'picro + frak2nM'))
...
BLA_sEPSC_final$Condition <- plyr::revalue(BLA_sEPSC_final$Condition, c("b"="baseline", "f" = "fractalkine"))
BLA_sEPSC_final$Condition <- factor(BLA_sEPSC_final$Condition, levels = c("baseline", "fractalkine", "AZD", "minocycline"))
BLA_sEPSC_final$Experiment = factor(BLA_sEPSC_final$Experiment, levels=c('frak2nM', 'frak5nM','antibody + frak2nM', 'AZD + frak2nM','AZD', 'mino + frak2nM', 'minocycline', 'picro + frak2nM'))
BLA_sEPSC_final$Cell <- rep(1:(nrow(BLA_sEPSC_final)/2), each = 2)


# previous file - all conditions
...
# BLA_sIPSC_final$Condition <- plyr::revalue(BLA_sIPSC_final$Condition, c("b"="baseline", "f" = "fractalkine"))
# BLA_sIPSC_final$Condition <- factor(BLA_sIPSC_final$Condition, levels = c("baseline", "fractalkine", "AZD", "minocycline"))
# BLA_sIPSC_final$Experiment = factor(BLA_sIPSC_final$Experiment, levels=c('frak2nM', 'frak5nM','antibody + frak2nM','AZD + frak2nM','AZD + frak5nM', 'AZD','mino + frak2nM', 'mino + frak5nM', 'minocycline'))
# BLA_sIPSC_final$Cell <- rep(1:(nrow(BLA_sIPSC_final)/2), each = 2)

# for publication and PhD - only important conditions
BLA_sIPSC_final <- BLA_frak2nM_sIPSC %>% dplyr::bind_rows(BLA_frak5nM_sIPSC) %>%  dplyr::bind_rows(BLA_minofrak5nM_sIPSC) %>% dplyr::bind_rows(BLA_minoa_sIPSC) %>% dplyr::bind_rows(BLA_azda_sIPSC) %>% dplyr::bind_rows(BLA_azdfrak5nM_sIPSC)
BLA_sIPSC_final <- BLA_sIPSC_final %>% dplyr::select(Experiment, Filename, Cell, Condition, everything()) %>% as_tibble()
BLA_sIPSC_final$Condition <- plyr::revalue(BLA_sIPSC_final$Condition, c("b"="baseline", "f" = "fractalkine"))
BLA_sIPSC_final$Condition <- factor(BLA_sIPSC_final$Condition, levels = c("baseline", "fractalkine", "AZD", "minocycline"))
BLA_sIPSC_final$Experiment = factor(BLA_sIPSC_final$Experiment, levels=c('frak2nM', 'frak5nM','AZD + frak5nM', 'AZD', 'mino + frak5nM', 'minocycline'))
BLA_sIPSC_final$Cell <- rep(1:(nrow(BLA_sIPSC_final)/2), each = 2)

# for appendix/supplementary material - to do
# BLA_sIPSC_final <- BLA_frak2nM_sIPSC %>% dplyr::bind_rows(BLA_frak5nM_sIPSC) %>% dplyr::bind_rows(BLA_azd_sIPSC) %>% dplyr::bind_rows(BLA_anti_sIPSC) %>% dplyr::bind_rows(BLA_mino_sIPSC) %>% dplyr::bind_rows(BLA_minofrak5nM_sIPSC) %>% dplyr::bind_rows(BLA_minoa_sIPSC) %>% dplyr::bind_rows(BLA_azda_sIPSC) %>% dplyr::bind_rows(BLA_azdfrak5nM_sIPSC)
# BLA_sIPSC_final <- BLA_sIPSC_final %>% dplyr::select(Experiment, Filename, Cell, Condition, everything()) %>% as_tibble()
# BLA_sIPSC_final$Condition <- plyr::revalue(BLA_sIPSC_final$Condition, c("b"="baseline", "f" = "fractalkine"))
# BLA_sIPSC_final$Condition <- factor(BLA_sIPSC_final$Condition, levels = c("baseline", "fractalkine", "AZD", "minocycline"))
...
BLA_frak2nM_mEPSC <- BLA_frak2nM_mEPSC %>% mutate(Experiment = 'frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_anti_mEPSC <- BLA_anti_mEPSC %>% mutate(Experiment = 'antibody + frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_mino_mEPSC <- BLA_mino_mEPSC %>% mutate(Experiment = 'mino + frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_mEPSC_final$Condition <- plyr::revalue(BLA_mEPSC_final$Condition, c("b"="baseline", "f" = "fractalkine"))
BLA_mEPSC_final$Experiment = factor(BLA_mEPSC_final$Experiment, levels=c('frak2nM','antibody + frak2nM','mino + frak2nM'))
...
BLA_frak2nM_mIPSC <- BLA_frak2nM_mIPSC %>% mutate(Experiment = 'frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_frak5nM_mIPSC <- BLA_frak5nM_mIPSC %>%  mutate(Experiment = 'frak5nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
BLA_frak5nM_mIPSC$Filename <- as.character(BLA_frak5nM_mIPSC$Filename)

#BLA_azd_mIPSC <- BLA_azd_mIPSC %>% mutate(Experiment = 'AZD + frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
BLA_anti_mIPSC <- BLA_anti_mIPSC %>% mutate(Experiment = 'antibody + frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
BLA_anti_mIPSC$Filename <- as.character(BLA_anti_mIPSC$Filename)
#BLA_mino_mIPSC <- BLA_mino_mIPSC %>% mutate(Experiment = 'mino + frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_mIPSC_final$Condition <- plyr::revalue(BLA_mIPSC_final$Condition, c("b"="baseline", "f" = "fractalkine"))
BLA_mIPSC_final$Experiment = factor(BLA_mIPSC_final$Experiment, levels=c('frak2nM', 'frak5nM'))
...
BLA_frak2nM_sEPSC_inter <- BLA_frak2nM_sEPSC_inter %>% mutate(Experiment = 'frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_frak5nM_sEPSC_inter <- BLA_frak5nM_sEPSC_inter %>% mutate(Experiment = 'frak5nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_mino_sEPSC_inter <- BLA_mino_sEPSC_inter  %>% mutate(Experiment = 'mino + frak2nM') %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
BLA_sEPSC_final_inter$Condition <- plyr::revalue(BLA_sEPSC_final_inter$Condition, c("b"="baseline", "f" = "fractalkine"))
###############################################################################################################################################################################################################################################################################################################################################################LTP###################################################################
...
df_basic$Condition <- plyr::revalue(df_basic$Condition, c("FRA 2 ADX 10uM"="AZD + frak2nM", "fra 2nM" = "frak2nM"))
df_basic$Time <-as.numeric(levels(df_basic$Time))[df_basic$Time]
df_basic$Condition <- factor(df_basic$Condition, levels = c("naive", "frak2nM", "AZD + frak2nM"))
...
bartek_IV2$Condition <- plyr::revalue(bartek_IV2$Condition, c("ADXcontrol"="control", "fraccontrol" = "control", "ADXFRXcon" = "control", "ADX10uM" = "AZD + frak2nM", "frac2nM" = "frak2nM", "ADX10FRX2" = "AZD + frak2nM"))
df_basic <- Rmisc::summarySE(bartek_IV2 , measurevar="Amplitude", groupvars=c("Condition","Stim"))
df_basic <- as_tibble(df_basic)
df_basic$Stim <-as.numeric(levels(df_basic$Stim))[df_basic$Stim]
df_basic$Condition <- factor(df_basic$Condition, levels = c("control",  "frak2nM", "AZD", "AZD + frak2nM"))
...
bartek_pp2$Condition <- plyr::revalue(bartek_pp2$Condition, c("naive"="control","ADX 10 FRC 2"="AZD + frak2nM", "adx 10uM" = "AZD + frak2nM", "ADX 10um" = "AZD + frak2nM" ,"fra con" = "control", "adx control" = "control",  "fra 2nM" = "frak2nM", "ADX FRC control" = "control"))
bartek_pp2$Condition <- factor(bartek_pp2$Condition, levels = c("control", "frak2nM", 'AZD', "AZD + frak2nM"))
