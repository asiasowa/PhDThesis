...
CeA_df_fluoro <- CeA_df_fluoro %>% mutate(Experiment = "fluorocitrate + CXCL12(5nM)") %>% dplyr::select(Experiment, Filename_all1, Cell, Condition, Type, everything()) %>% dplyr::rename(sag_ratio_140 = sag_ratio)
bad_Cells <- c(1)
...
CeA_sEPSC_fluoro <-CeA_fluoro_sEPSC_LF %>% dplyr::mutate(Experiment = "fluorocitrate + CXCL12(5nM)") %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
...
CeA_sIPSC_fluoro_5nM <- CeA_sIPSC_fluoro_5nM_RS %>% bind_rows(CeA_sIPSC_fluoro_5nM_LF) %>% mutate(Experiment = "fluorocitrate + CXCL12(5nM)") %>% dplyr::select(Experiment, Filename, Cell, Condition, everything())
