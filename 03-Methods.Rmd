...
Only recordings with stable access resistance lower than 20 M`r knitr::asis_output("\U038F")` were considered acceptable for analysis. 
...
An input resistance (R~m~), a membrane time constant (`r knitr::asis_output("\U03C4")`~m~), and a membrane capacitance (C~m~) were determined in response to hyperpolarizing current pulses eliciting voltage shifts whose amplitude was 10–20 mV negative to rest.<!-- (Kawaguchi 1993) // inne cytowanie--> 
...
An action potential threshold corresponded to the voltage threshold of the first action potential elicited by a current step, defined as the voltage at which the time derivative of V~m~($\frac{\partial V}{\partial t}$) is 20 V s^−1^.
...
CX3CR1 localization to neurons was determined by a sequential incubation of brain slices with a rabbit anti-CX3CR1 antibody (1:100; Torrey Pines Biolabs, USA) with a mouse anti-CAMKII`r knitr::asis_output("\U03B1")` (1:1000; Invitrogen, USA).
...
Three-channel images were acquired DAPI, CX3CR1 and CAMKII`r knitr::asis_output("\U03B1")`. 
...
The total numbers of positively identified cells expressing CX3CR1 counted manually in individual brain sections.<!-- from 3 rats in each treatment group. -->
The number of cells was counted in the basolateral amygdala.
All DAPI-labeled cells expressing markers (CAMKII`r knitr::asis_ouput("\U03B1")`) were counted in the BLA or CeA and subsequently classified as "total CAMK+-labeled" cells. 
