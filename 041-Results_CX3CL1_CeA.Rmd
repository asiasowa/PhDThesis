...
Intriguingly, majority of RS neurons were revealed to respond to CX3CL1, which was reflected as robust changes in intrinsic properties (Figs. \@ref(fig:CX3CL1CeAVmexc) and \@ref(fig:CX3CL1Rmtau), middle panels).
In contrast, CX3CL1 seems not to affect LF cells' intrinsic membrane properties (see Figs. \@ref(fig:CX3CL1CeAVmexc) and \@ref(fig:CX3CL1Rmtau), right panels), implying that much higher proportion of RS cells was reactive to CX3CL1.
...
bp_CeA_vm <- plot_boxplot(CeA_df_final_sub, RMS, mtitle= "CeL - \nAll Neurons", ylabel = "Resting membrane potential (mV)", vary1 = -80, vary2 = -25, dot_size = 2)
...
# GAIN
bp_CeA_gain <- plot_boxplot(CeA_df_final_sub, gain, mtitle= "", ylabel = "gain",  dot_size = 2)
bp_CeA_gain <- bp_CeA_gain + facet_wrap(~ Experiment, nrow = 1, scales = "free_x") + theme(plot.title = element_text(size= 16, face="bold"), axis.text.x = element_text(angle = 30, vjust = 0.7, hjust=0.5),
 strip.text = element_text(size = 11, face="bold"), strip.background = element_blank()) + stat_compare_means(method = 't.test', hide.ns = TRUE, label = 'p.signif', paired = TRUE, label.x.npc = 'center', var.equal = TRUE,  size = 10, face = "bold", label.y = 0.3)
...
A <- ((bp_CeA_vm | bp_CeA_vm_RS | bp_CeA_vm_LF) + plot_layout(tag_level = "new"))
B <- ((bp_CeA_gain | bp_CeA_gain_RS | bp_CeA_gain_LF) + plot_layout(tag_level = "new"))
...
```{r CX3CL1CeARmtau, fig.cap='**Effects of CX3CL1 on input resistance and membrane time constant** **(a1-3)** Boxplots showing input resistance of each CeL neuron in general, Regular-Spiking neuron, or Late-Firing neuron, respectively, before and after CX3CL1 application. **(b1-3)** Boxplots illustrating membrane time constant (tau) of each CeL neuron in general, Regular-Spiking neuron, or Late-Firing neuron, respectively, before and after CX3CL1 application. **(c1)** Aligned voltage traces from a representative cell to -20pA injection currents prior to (blue) and after the CX3CL1 application (red) illustrating changes in the input resistance and membrane time constant. **(c2)** Aligned voltage traces from a representative cell to -20pA injection currents prior to (blue) and after the CX3CL1 application (red) illustrating changes in the input resistance and membrane time constant in the presence of AZD8797, a blocker of CX3CL1 receptor. **(c3)** Aligned voltage traces from a representative cell to -20pA injection currents prior to (blue) and after the CX3CL1 application (red) illustrating changes in the input resistance and membrane time constant in the presence of minocycline, a blocker of microglial activity. Scale bar: 10mV, 100ms', fig.align = 'center', echo = FALSE, message= FALSE, warning= FALSE, fig.width= 12, fig.height= 8}
...
# # INPUT RESISTANCE
bp_CeA_rm <- plot_boxplot(CeA_df_final_sub, Rm_20, mtitle= "CeL - \nAll Neurons", ylabel = "Input resistance (MOhm)", vary1 = 100, vary2 = 900, dot_size = 2)
...
#using patchwork to do layout of the figure
A <- ((bp_CeA_rm | bp_CeA_rm_RS | bp_CeA_rm_LF) + plot_layout(tag_level = "new"))
B <- ((bp_CeA_tau | bp_CeA_tau_RS | bp_CeA_tau_LF) + plot_layout(tag_level = "new"))
