...
However, since CeL neurons demonstrate mainly two types of electrophysiological firing patterns, such as Regular-Spiking and Late-Firing neurons (see: Introduction \@ref(CeA) and Fig. \@ref(fig:methodsBLACeAIVrep)b), these intrinsic parameters were further analyzed based on these neuronal subtypes.
...
Specifically, treatment with 20`r knitr::asis_output("\U03BC")`M minocycline lowered the threshold needed for inducing action potentials in both types of neurons (from `r round(CeA_df_final %>% filter(Experiment == 'minocycline' & Type == 'LF' & Condition == "baseline") %>% ungroup() %>% summarize(mean(Treshold1_all)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_df_final %>% filter(Experiment == 'minocycline' & Type == 'LF' & Condition == "baseline") %>% ungroup() %>% dplyr::summarize(x = sd(Treshold1_all), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` mV to `r round(CeA_df_final %>% filter(Experiment == 'minocycline' & Type == 'LF' & Condition == "minocycline") %>% ungroup() %>% summarize(mean(Treshold1_all)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_df_final %>% filter(Experiment == 'minocycline' & Type == 'LF' & Condition == "minocycline") %>% ungroup() %>% dplyr::summarize(x = sd(Treshold1_all), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` mV, `r mino_CeA_stat_LF_tres` in Late Firing neurons 
