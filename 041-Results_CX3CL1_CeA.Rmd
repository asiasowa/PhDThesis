...
#### CX3CL1 affects electrophysiological membrane properties of CeL neurons in the cell-type specific manner

```{r CX3CL1CeAexcitabilitystats, include = FALSE}
# getting the table of statistics for all condition of one experiment
CX3CL1_CeA_stat_2nM <- (stat_table_exp_membr_extended((CeA_df_final %>% filter(Experiment == 'CX3CL1(2nM)'))))
# selecting a stat_row so it prints nicely in text
CX3CL1_CeA_stat_2nM_gain <- as.data.frame(CX3CL1_CeA_stat_2nM %>% dplyr::filter(Variable == 'gain') %>% dplyr::select(Statistics))

CX3CL1_CeA_stat_2nM_Rm <- as.data.frame(CX3CL1_CeA_stat_2nM %>% dplyr::filter(Variable == 'Input_resistance') %>% dplyr::select(Statistics)) 

CX3CL1_CeA_stat_2nM_tau <- as.data.frame(CX3CL1_CeA_stat_2nM %>% dplyr::filter(Variable == 'tau') %>% dplyr::select(Statistics))

CX3CL1_CeA_stat_2nM_Vm <- as.data.frame(CX3CL1_CeA_stat_2nM %>% dplyr::filter(Variable == 'Vm') %>% dplyr::select(Statistics))
...
To this end, among others, such electrophysiological features as the excitability, input resistance, membrane time constant, resting membrane potential, was measured (see \@ref(electroprotocols)). <!-- analogous to the neurons in the BLA./// recorded using a whole-cell patch-clamp approach, in a current-clamp configuration.-->

Findings presented in this PhD thesis revealed that, in general, neurons reacted with reduced excitability, manifested as decreased gain.
In addition, the input resistance was also decreased after CX3CL1 application, together with membrane time constant.
Moreover, those cells were more depolarized. 

Given that CeL comprises neurons of two main electrophysiological types, further analysis segregated those neurons according to their types.<!-- odnośnik do góry i lepiej napisać-->
Intriguingly, this additional analysis revealed that those parameters were changed specifically in the Regular-Spiking (RS) neurons, but not in the Late-Firing. 
