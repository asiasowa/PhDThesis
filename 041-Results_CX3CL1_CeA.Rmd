...
```{r CeACX3CL1immunostat, echo = FALSE, warning = FALSE, message= FALSE}



cx3cr1_pkc <-  results_immuno %>% filter(Structure == "CeA") %>% dplyr::filter(Marker1 == "CX3CR1") %>% dplyr::filter(Marker2 == "Pkdelta") 
cx3cr1_pkc_col <- mean(cx3cr1_pkc$mean_col)
cx3cr1_pkc_sd <- sd(cx3cr1_pkc$mean_col)

cx3cr1_som <-  results_immuno %>% filter(Structure == "CeA") %>% dplyr::filter(Marker1 == "CX3CR1") %>% dplyr::filter(Marker2 == "SOM") 
cx3cr1_som_col <- mean(cx3cr1_som$mean_col)
cx3cr1_som_sd <- sd(cx3cr1_som$mean_col)

cx3cr1_gfap <-  results_immuno %>% filter(Structure == "CeA") %>% dplyr::filter(Marker1 == "CX3CR1") %>% dplyr::filter(Marker2 == "GFAP") 
cx3cr1_gfap_col <- mean(cx3cr1_gfap$mean_col)
cx3cr1_gfap_sd <- sd(cx3cr1_gfap$mean_col)
```

Having found that mainly Regular-Spiking neurons are reactive to CX3CL1 application, as assessed by changes in input resistance, time membrane constant, and excitability, the next step was to examine whether these cell-specific effects are reflected in different CX3CR1 expressions.
Unfortunately, neither Late-Firing nor Regular-Spiking electrophysiological type does clearly correlate with any specific molecular marker.
According to the literature, around 15% of Regular-Spiking neurons express PKC`r knitr::asis_output("\U03B4")`, whereas around 60% of all SOM-immunopositive neurons belong to the RS type [@haubensak_2010; @li_2013a].
Thus, it was hypothesized that observed changes in membrane properties in RS following CX3CL1 application are associated with the presence of CX3CR1 predominantly in SOM-positive neurons.
