...
To study whether BLA network can be regulated by CXCL12, whole-cell patch-clamp recordings were obtained from principal cells.
...
Moreover, other measured membrane intrinsic properties of principal cells were also unaffected by CXCL12 treatment, as shown in Fig. \@ref(fig:CXCl12BLAexcitabilityfigall) in Appendix \@ref(BLACXCL12membrappend)).
...
These findings also need further replication and investigation of the potential mechanisms underlying opposite effects of CXCL12 on sEPSCs frequency.
...
Although none of the measured sIPSCs' parameters reached statistical significance, there is a trend in the frequency.
As shown in Fig.\@ref(fig:CXCL12BLAsIPSCfreq), at the concentration of 2nM, CXCL12 reduced the GABAergic transmission onto `r round((pie_BLA_c2_freq$data$count[1]/(pie_BLA_c2_freq$data$count[1] + pie_BLA_c2_freq$data$count[2] + pie_BLA_c2_freq$data$count[3]))*100, digits = 2)`% of principal cells.
In contrast, at 5nM, `r pie_BLA_c5_freq$data$count[2]` out of `r (pie_BLA_c5_freq$data$count[1] + pie_BLA_c5_freq$data$count[2] + pie_BLA_c5_freq$data$count[3])` neurons showed an increased frequency of sIPSCs. 
In addition, sIPSCs' rise time is trendily increased by both doses of CXCL12 (Fig.\@ref(fig:CXCL12BLAsIPSCfreq)b and d).
However, this multifaceted effect needs further replication and exploration.
...
First, to investigate the potential involvement of CXCL12 in neuronal activity in the CeL, the intrinsic firing properties of CeL neurons were assessed through whole-cell patch-clamp recordings, evoking action potentials in response to somatic current injections.
Data reported in Figure \@ref(fig:CXCL12CeAexc) demonstrates that CXCL12, especially at a higher concentration (5nM), enhanced CeL neurons' excitability, measured as threshold current required for the first action potential and gain(slope) as a function of injected current and frequency of firing.
As described earlier, two main electrophysiological types of CeL neurons are Regular-Spiking and Late-Firing neurons.
Thus, further analysis revealed that although this CXCL12-induced increased firing can be observed in both types of cells (Fig. \@ref(fig:CXCL12CeAexc), right panel), it is more prominent and statistically significant in Late-Firing neurons, with threshold current during baseline being `r round(CeA_df_final_12_LF %>% filter(Experiment == 'CXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% summarize(mean(Treshold1_all)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_df_final_12_LF %>% filter(Experiment == 'CXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% dplyr::summarize(x = sd(Treshold1_all), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` mV and reaching `r round(CeA_df_final_12_LF %>% filter(Experiment == 'CXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% summarize(mean(Treshold1_all)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_df_final_12_LF %>% filter(Experiment == 'CXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% dplyr::summarize(x = sd(Treshold1_all), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` mV after CXCL12(5nM) treatment, n = `r CeA_df_final_12_LF %>% filter(Experiment == 'CXCL12(5nM)') %>% ungroup() %>% dplyr::select(Cell) %>% n_distinct()`, N = `r CeA_df_final_12_LF %>% filter(Experiment == 'CXCL12(5nM)') %>% group_by(Filename_all1) %>% mutate(animals = str_sub(Filename_all1, start = 1, end = 5)) %>% ungroup() %>% dplyr::select(animals) %>% n_distinct()`, `r CXCL12_CeA_stat_LF_thres$Statistics[2]`).
These effects were prevented, at least partially, by the selective CXCR4 antagonist, AMD3100 (baseline in AMD3100: `r round(CeA_df_final_12_LF %>% filter(Experiment == 'AMD3100 + \nCXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% summarize(mean(Treshold1_all)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_df_final_12_LF %>% filter(Experiment == 'AMD3100 + \nCXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% dplyr::summarize(x = sd(Treshold1_all), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` mV, CXCL12(5nM) in AMD3100 `r round(CeA_df_final_12_LF %>% filter(Experiment == 'AMD3100 + \nCXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% summarize(mean(Treshold1_all)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_df_final_12_LF %>% filter(Experiment == 'AMD3100 + \nCXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% dplyr::summarize(x = sd(Treshold1_all), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` mV, n = `r CeA_df_final_12_LF %>% filter(Experiment == 'AMD3100 + \nCXCL12(5nM)') %>% ungroup() %>% dplyr::select(Cell) %>% n_distinct()`, N = `r CeA_df_final_12_LF %>% filter(Experiment == 'AMD3100 + \nCXCL12(5nM)') %>% group_by(Filename_all1) %>% mutate(animals = str_sub(Filename_all1, start = 1, end = 5)) %>% ungroup() %>% dplyr::select(animals) %>% n_distinct()`, `r CXCL12_CeA_stat_amd5nM_LF_thres`).
Of note, the application of AMD3100 alone led to an opposite trend (from `r round(CeA_df_final_12_LF %>% filter(Experiment == 'AMD3100' & Condition == "baseline") %>% ungroup() %>% summarize(mean(Treshold1_all)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_df_final_12_LF %>% filter(Experiment == 'AMD3100' & Condition == "baseline") %>% ungroup() %>% dplyr::summarize(x = sd(Treshold1_all), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` mV to `r round(CeA_df_final_12_LF %>% filter(Experiment == 'AMD3100' & Condition == "AMD3100") %>% ungroup() %>% summarize(mean(Treshold1_all)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_df_final_12_LF %>% filter(Experiment == 'AMD3100' & Condition == "AMD3100") %>% ungroup() %>% dplyr::summarize(x = sd(Treshold1_all), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` mV, n = `r CeA_df_final_12_LF %>% filter(Experiment == 'AMD3100') %>% ungroup() %>% dplyr::select(Cell) %>% n_distinct()`, N = `r CeA_df_final_12_LF %>% filter(Experiment == 'AMD3100') %>% group_by(Filename_all1) %>% mutate(animals = str_sub(Filename_all1, start = 1, end = 5)) %>% ungroup() %>% dplyr::select(animals) %>% n_distinct()`,`r CXCL12_CeA_stat_amda_LF_thres`, Fig.\@ref(fig:CXCL12CeAexc), right panel). 
...
This effect was not abolished by an astrocytic inactivation in an acute brain slice (`r round(round(CeA_df_final_12_LF %>% filter(Experiment == 'fluorocitrate + \nCXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% summarize(mean(Treshold1_all)), digits = 2)/round(CeA_df_final_12_LF %>% filter(Experiment == 'fluorocitrate + \nCXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% summarize(mean(Treshold1_all)), digits = 2), digits = 2)` `r knitr::asis_output("\U00B1")``r round((round(CeA_df_final_12_LF %>% filter(Experiment == 'fluorocitrate + \nCXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% dplyr::summarize(x = sd(Treshold1_all), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)) / (round(CeA_df_final_12_LF %>% filter(Experiment == 'fluorocitrate + \nCXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% dplyr::summarize(x = sd(Treshold1_all), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)), digits = 2)`% of baseline, 
`r CXCL12_CeA_stat_fc5nM_LF_thres`), raising the possibility that CXCL12 might activate CXCR4 directly on Late-Firing neurons, enhancing the excitability of these neurons.
Notably, this observation is based on a limited number of neurons (n = `r CeA_df_final_12_LF %>% filter(Experiment == 'fluorocitrate + \nCXCL12(5nM)') %>% ungroup() %>% dplyr::select(Cell) %>% n_distinct()`, N = `r CeA_df_final_12_LF %>% filter(Experiment == 'fluorocitrate + \nCXCL12(5nM)') %>% group_by(Filename_all1) %>% mutate(animals = str_sub(Filename_all1, start = 1, end = 5)) %>% ungroup() %>% dplyr::select(animals) %>% n_distinct()`) and, therefore, this finding needs further replication.
...
```{r CXCL12CeAexc, fig.cap='**CXCL12 enhanced excitability of CeL neurons** **(a1)** Boxplots showing a gain of each cell before and after CXCL12 application in Regular-Spiking neurons. **(a2)** Boxplots showing a gain of each cell before and after CXCL12 application in Late-Firing neurons across different conditions. **(b1)** Boxplots showing threshold current of each cell before and after CXCL12 application in Regular-Spiking neurons. **(b2)** Boxplots showing threshold current of each cell before and after CXCL12 application in Late-Firing neurons across different conditions. **(c1)** Input-Output relationship of a representative neuron illustrating its excitability before (baseline) and after CXCL12 treatment. **(c2)** Voltage traces from the same representative cells presented as orange dots on the previous plot. **(d1)** Input-Output relationship of a representative neuron illustrating its excitability before (baseline) and after CXCL12 treatment in the presence of AMD3100, the blocker of CXCR4 receptor. **(d2)** Voltage traces from the same representative cells presented as orange dots on the previous plot. **(e1)** Input-Output relationship of a representative neuron illustrating its excitability before (baseline) and after CX3CL1 treatment in the presence of fluorocitrate, the blocker of astrocytic activity. **(d2)** Voltage traces from the same representative cells presented as orange dots on the previous plot. Scale bar: 10mV, 100ms', fig.align = 'center', echo = FALSE, message= FALSE, warning= FALSE, fig.width= 15, fig.height= 12}
...
## cxcl12
CXCL12_IV_subset <- CeA_df_final_12_LF %>% filter(Experiment == "CXCL12(5nM)" & Cell %in% c(14)) # subtracting one representative cell from whole dataframe
CXCL12_IV_subset$Filename_all1[CXCL12_IV_subset$Filename_all1 == "20d09017"] = "" #renaming the name - instead the name of the file - the name of the condition
IV_representative_cxcl12_5nM <- CXCL12_IV_subset %>% plot_IV_individual(mtitle = "") + theme(strip.text = element_text(size = 11, face="bold"), strip.background = element_blank(), plot.margin = unit(c(0.25,0.25,0.25,0.25), "cm")) + annotate(geom = "point", x = 30, y = 2, size = 4 , colour = "orange", alpha = 0.5) + annotate(geom = "point", x = 30, y = 8, size = 4 , colour = "orange", alpha = 0.5) + annotate(geom = "point", x = 70, y = 14, size = 4 , colour = "orange", alpha = 0.5) + annotate(geom = "point", x = 70, y = 12, size = 4 , colour = "orange", alpha = 0.5) + annotate(geom = "point", x = 110, y = 16, size = 4 , colour = "orange", alpha = 0.5)
# ## importing traces  
amdCXCL12_IV_subset <- CeA_df_final_12_LF %>% filter(Experiment == "AMD3100 + \nCXCL12(5nM)" & Cell %in% c(4)) # subtracting one representative cell from whole dataframe
amdCXCL12_IV_subset$Filename_all1[amdCXCL12_IV_subset$Filename_all1 == "20d14053"] = "" #renaming the name - instead the name of the file - the name of the condition
representative_amd_5nM <- amdCXCL12_IV_subset %>% plot_IV_individual(mtitle = "") + theme(strip.text = element_text(size = 11, face="bold"), strip.background = element_blank(), plot.margin = unit(c(0.25,0.25,0.25,0.25), "cm")) + annotate(geom = "point", x = 40, y = 2, size = 4 , colour = "orange", alpha = 0.5) + annotate(geom = "point", x = 40, y = 2, size = 4 , colour = "orange", alpha = 0.5) + annotate(geom = "point", x = 90, y = 14, size = 4 , colour = "orange", alpha = 0.5) + annotate(geom = "point", x = 90, y = 16, size = 4 , colour = "orange", alpha = 0.5)  + annotate(geom = "point", x = 130, y = 22, size = 4 , colour = "orange", alpha = 0.5) + annotate(geom = "point", x = 130, y = 24, size = 4 , colour = "orange", alpha = 0.5)
fluoroCXCL12_IV_subset <- CeA_df_final_12_LF %>% filter(Experiment == "fluorocitrate + \nCXCL12(5nM)" & Cell %in% c(2)) # subtracting one representative cell from whole dataframe
fluoroCXCL12_IV_subset$Filename_all1[fluoroCXCL12_IV_subset$Filename_all1 == "21507033"] = "" #renaming the name - instead the name of the file - the name of the condition
representative_fluoro_5nM <- fluoroCXCL12_IV_subset %>% plot_IV_individual(mtitle = "") + theme(strip.text = element_text(size = 11, face="bold"), strip.background = element_blank(), plot.margin = unit(c(0.25,0.25,0.25,0.25), "cm"))  + annotate(geom = "point", x = 20, y = 2, size = 4 , colour = "orange", alpha = 0.5) + annotate(geom = "point", x = 70, y = 14, size = 4 , colour = "orange", alpha = 0.5) + annotate(geom = "point", x = 70, y = 12, size = 4 , colour = "orange", alpha = 0.5) + annotate(geom = "point", x = 130, y = 20, size = 4 , colour = "orange", alpha = 0.5) + annotate(geom = "point", x = 130, y = 18, size = 4 , colour = "orange", alpha = 0.5)
...
traces_cxcl12_5nM <- fig("images//results//CXCL12_CeA_IV_LF_cxcl12.png", link_dim = F, aspect.ratio = 'free')

traces_amd_5nM <- fig("images//results//CXCL12_CeA_IV_LF_azd.png", link_dim = F, aspect.ratio = 'free')

traces_fluoro_5nM <- fig("images//results//CXCL12_CeA_IV_LF_fluoro.png", link_dim = F, aspect.ratio = 'free')
...
C <- (((IV_representative_cxcl12_5nM +  ggtitle('CXCL12(5nM)'))| traces_cxcl12_5nM) + plot_layout(tag_level = "new", widths = c(0.5,1))) 
D <- (((representative_amd_5nM +  ggtitle('AMD3100 + CXCL12(5nM)'))| traces_amd_5nM) + plot_layout(tag_level = "new", widths = c(0.5,1))) 
E <- (((representative_fluoro_5nM +  ggtitle('fluorocitrate + CXCL12(5nM)')) | traces_fluoro_5nM) + plot_layout(tag_level = "new", widths = c(0.5,1))) 
...
Intriguingly, regarding excitatory synaptic transmission onto Regular-Firing neurons, two opposing trends (decreased vs. increased frequency of sEPSCs) were revealed depending on the concentration of CXCL12 used (2nM vs. 5nM; see Fig. (\@ref(fig:CXCL12CeAsEPSC)a2)).
...
A detailed summary of measured parameters of excitatory synaptic transmission, together with Figure \@ref(fig:CXCL12CeAsEPSCappen) illustrating the amplitude and kinetics of sEPSCs after CXCL12 treatment can be found in Table 7.11 in Appendix \@ref(CXCL12CeAsEPSCappend).
...
Specifically, CXCL12 enhanced the GABAergic transmission onto Regular-Spiking neurons, as manifested by increased frequency of sIPSCs from `r round(CeA_sIPSC_RS_subset %>% filter(Experiment == 'CXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% summarize(mean(median_Frequency)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_sIPSC_RS_subset %>% filter(Experiment == 'CXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% dplyr::summarize(x = sd(median_Frequency), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` Hz to `r round(CeA_sIPSC_RS_subset  %>% filter(Experiment == 'CXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% summarize(mean(median_Frequency)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_sIPSC_RS_subset %>% filter(Experiment == 'CXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% dplyr::summarize(x = sd(median_Frequency), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` Hz (n = `r CeA_sIPSC_RS_subset %>% filter(Experiment == 'CXCL12(5nM)') %>% ungroup() %>% dplyr::select(Cell) %>% n_distinct()`, N = `r CeA_sIPSC_RS_subset %>% filter(Experiment == 'CXCL12(5nM)') %>% group_by(Filename) %>% mutate(animals = str_sub(Filename, start = 1, end = 5)) %>% ungroup() %>% dplyr::select(animals) %>% n_distinct()`, `r CeA_sIPSC_RS_stats_freq_5nM`, Fig.\@ref(fig:CXCL12CeAsIPSCRS)a1), 
...
As expected, AMD3100 eliminated the CXCL12-induced increase in the frequency of sIPSC, with `r round(CeA_sIPSC_RS_subset %>% filter(Experiment == 'AMD3100 + \nCXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% summarize(mean(median_Frequency)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_sIPSC_RS_subset %>% filter(Experiment == 'AMD3100 + \nCXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% dplyr::summarize(x = sd(median_Frequency), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` Hz in baseline
and `r round(CeA_sIPSC_RS_subset %>% filter(Experiment == 'AMD3100 + \nCXCL12(5nM)' & Condition == 'CXCL12') %>% ungroup() %>% summarize(mean(median_Frequency)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_sIPSC_RS_subset %>% filter(Experiment == 'AMD3100 + \nCXCL12(5nM)' & Condition == 'CXCL12') %>% ungroup() %>% dplyr::summarize(x = sd(median_Frequency), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` Hz after CXCL12 treatment (`r CeA_sIPSC_RS_stats_freq_amd5nM`, n = `r CeA_sIPSC_RS_subset %>% filter(Experiment == 'AMD3100 + \nCXCL12(5nM)') %>% ungroup() %>% dplyr::select(Cell) %>% n_distinct()`, N = `r CeA_sIPSC_RS_subset %>% filter(Experiment == 'AMD3100 + \nCXCL12(5nM)') %>% group_by(Filename) %>% mutate(animals = str_sub(Filename, start = 1, end = 5)) %>% ungroup() %>% dplyr::select(animals) %>% n_distinct()`, Fig.\@ref(fig:CXCL12CeAsIPSCRS)b2).
Since CXCR4 is well-known to be functionally expressed on astrocytes, the possible involvement of astrocytes in mediating CXCL12-induced enhancement of GABAergic transmission onto Regular-Spiking neurons was investigated.
...
As shown in Fig. \@ref(fig:CXCL12CeAsIPSCRS)b3, fluorocitrate prevented the CXCL12-triggered increase in the frequency of sIPSCs (`r round(round(CeA_sIPSC_RS_subset %>% filter(Experiment == 'fluorocitrate + \nCXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% summarize(mean(median_Frequency)), digits = 2)/round(CeA_sIPSC_RS_subset %>% filter(Experiment == 'fluorocitrate + \nCXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% summarize(mean(median_Frequency)), digits = 2), digits = 2)` `r knitr::asis_output("\U00B1")``r round((round(CeA_sIPSC_RS_subset %>% filter(Experiment == 'fluorocitrate + \nCXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% dplyr::summarize(x = sd(median_Frequency), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)) / (round(CeA_sIPSC_RS_subset %>% filter(Experiment == 'fluorocitrate + \nCXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% dplyr::summarize(x = sd(median_Frequency), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)), digits = 2)`% of baseline,
`r CeA_sIPSC_RS_stats_freq_fluoro5nM`, n = `r CeA_sIPSC_RS_subset %>% filter(Experiment == 'fluorocitrate + \nCXCL12(5nM)') %>% ungroup() %>% dplyr::select(Cell) %>% n_distinct()`, N = `r CeA_sIPSC_RS_subset %>% filter(Experiment == 'fluorocitrate + \nCXCL12(5nM)') %>% group_by(Filename) %>% mutate(animals = str_sub(Filename, start = 1, end = 5)) %>% ungroup() %>% dplyr::select(animals) %>% n_distinct()`).

To determine whether there was a direct effect of CXCL12 on GABAergic terminals, the mIPSCs were recorded.
In these experiments, TTX was used to block the action potentials. 
Under these conditions, the modulation of GABAergic transmission by CXCL12 was partially abolished (mIPSC frequency during baseline:  `r round(CeA_sIPSC_RS_subset %>% filter(Experiment == 'TTX + \nCXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% summarize(mean(median_Frequency)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_sIPSC_RS_subset %>% filter(Experiment == 'TTX + \nCXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% dplyr::summarize(x = sd(median_Frequency), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` Hz vs. mIPSC frequency after CXCL12 treatment: `r round(CeA_sIPSC_RS_subset %>% filter(Experiment == 'TTX + \nCXCL12(5nM)' & Condition == 'CXCL12') %>% ungroup() %>% summarize(mean(median_Frequency)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_sIPSC_RS_subset %>% filter(Experiment == 'TTX + \nCXCL12(5nM)' & Condition == 'CXCL12') %>% ungroup() %>% dplyr::summarize(x = sd(median_Frequency), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` Hz, `r CeA_sIPSC_RS_stats_freq_ttx5nM`, n = `r CeA_sIPSC_RS_subset %>% filter(Experiment == 'TTX + \nCXCL12(5nM)') %>% ungroup() %>% dplyr::select(Cell) %>% n_distinct()`, N = `r CeA_sIPSC_RS_subset %>% filter(Experiment == 'TTX + \nCXCL12(5nM)') %>% group_by(Filename) %>% mutate(animals = str_sub(Filename, start = 1, end = 5)) %>% ungroup() %>% dplyr::select(animals) %>% n_distinct()`, Fig. \@ref(fig:CXCL12CeAsIPSCRS)b4).

Altogether, these results suggest that the CXCL12-triggered increase in the frequency of sIPSCs was mediated through the stimulation of CXCR4 and probably associated with the increase in spontaneous firing of interneurons, as this effect is eliminated by TTX.
However, the inhibition of astrocytes by fluorocitrate was also shown to eliminate the previously described effect of CXCL12 on GABAergic transmission in Regular-Spiking neurons (see \@ref(fig:CXCL12CeAsIPSCRS)).

```{r CXCL12CeAsIPSCRS, fig.cap="**CXCL12 modulates GABAergic transmission onto Regular-Spiking neurons** **(a1-6)** Schematics of presented conditions. Boxplots presenting population data of sIPSCs frequency **(b1)** and amplitude **(b2)** under different conditions. **(c1-4)** Representative current traces illustrating inhibitory synaptic currents (upward deflections) before (blue) and after (red) the CXCL12 applicaiton across different conditions in a Regular-Firing neuron. Scale bar: 10pA, 1s", fig.align = 'center', echo = FALSE, message= FALSE, echo = FALSE, warning= FALSE, message = FALSE, fig.height= 10, fig.width= 15}
#knitr::include_graphics("images/results//CX3CL1_BLA_excitability.png")
#knitr::include_graphics("images/results//CX3CL1_BLA_excitability.png")

inset_cxcl12_2 <- fig("images//results//inset_frak2nM.png")

inset_cxcl12_5 <- fig("images//results//inset_frak5nM.png")

inset_amdcxcl12_5 <- fig("images//results//inset_azdfrak5nM.png")

inset_amd <- fig("images//results//inset_azd.png")

inset_astro_cxcl12_5 <- fig("images//results//inset_fluorocxcl12.png")

inset_ttxcxcl12_5 <- fig("images//results//inset_ttx_5nM.png")
IV_representative_cxcl12_5nM <- fig("images//results//cxcl12_cea_sIPSC_RS_cxcl12.png") # , , aspect.ratio = 'free'
# 
IV_representative_amdcxcl12_5nM <- fig("images//results//cxcl12_cea_sIPSC_RS_amdcxcl12.png") # , aspect.ratio = 'free'
# 
IV_representative_fluorocxcl12_5nM <- fig("images//results//cxcl12_cea_sIPSC_RS_fluorocxcl12.png") # , aspect.ratio = 'free'
# 
IV_representative_ttxcxcl12_5nM <- fig("images//results//cxcl12_cea_sIPSC_RS_ttxcxcl12.png") # aspect.ratio = 'free'

# loading needed subsets 

# making boxplots with facets

bp_CeA_freq_sIPSC_all <- plot_boxplot(CeA_sIPSC_all_subset, median_Frequency, ylabel = "IPSC Frequency (Hz)", vary1 = 0, vary2 = 3, dot_size = 2,  mtitle = "Late-Firing neurons") + theme(plot.title = element_text(size= 16, face="bold"), axis.text.x = element_text(angle = 30, vjust = 0.7, hjust=0.5)) 

bp_CeA_freq_sIPSC_all <- bp_CeA_freq_sIPSC_all + facet_wrap(~ Experiment, nrow = 1, scales = "free_x") + theme(plot.title = element_text(size= 14, face="bold"), axis.text.x = element_text(angle = 30, vjust = 0.7, hjust=0.5),  strip.text = element_text(size = 11, face="bold"), strip.background = element_blank()) + stat_compare_means(method = 't.test', hide.ns = TRUE, label = 'p.signif', paired = TRUE, label.x.npc = 'center', var.equal = TRUE, size = 8, label.y = 2.3) 


bp_CeA_freq_sIPSC_LF <- plot_boxplot(CeA_sIPSC_LF_subset, median_Frequency, ylabel = "IPSC Frequency (Hz)", vary1 = 0, vary2 = 3, dot_size = 2,  mtitle = "Late-Firing neurons") + theme(plot.title = element_text(size= 16, face="bold"), axis.text.x = element_text(angle = 30, vjust = 0.7, hjust=0.5)) 
...
bp_CeA_freq_sIPSC_RS <- plot_boxplot(CeA_sIPSC_RS_subset, median_Frequency, ylabel = "IPSC Frequency (Hz)", vary1 = 0, vary2 = 3, dot_size = 2,  mtitle = "Regular-Spiking neurons") + theme(plot.title = element_text(size= 16, face="bold"), axis.text.x = element_text(angle = 30, vjust = 0.7, hjust=0.5)) 
...
#using patchwork to do layout of the figure
A <- (inset_cxcl12_2 | inset_cxcl12_5 | inset_amdcxcl12_5 | inset_amd | inset_astro_cxcl12_5 | inset_ttxcxcl12_5) + plot_layout(tag_level = "new")
B <- (bp_CeA_freq_sIPSC_RS / bp_CeA_amp_sIPSC_RS) + plot_layout(tag_level = "new")
#Y <- A / B
C <- (IV_representative_cxcl12_5nM) + ggtitle("CXCL12(5nM)")
D <- (IV_representative_amdcxcl12_5nM) + ggtitle("AMD3100 + \nCXCL12(5nM)")
E <- (IV_representative_fluorocxcl12_5nM) + ggtitle("fluorocitrate + \nCXCL12(5nM)")
G <- (IV_representative_ttxcxcl12_5nM) + ggtitle("TTX + CXCL12(5nM)")
X <- (C / D / E / G) + plot_layout(tag_level = "new")
...
figure1 <- wrap_plots(A = A, B = B, X = X, design = lay)
...
In contrast, in Late-Firing neurons, CXCL12 significantly reduced the sIPSC amplitude from `r round(CeA_sIPSC_LF_subset %>% filter(Experiment == 'CXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% summarize(mean(median_Amplitude)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_sIPSC_LF_subset %>% filter(Experiment == 'CXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% dplyr::summarize(x = sd(median_Amplitude), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` pA to `r round(CeA_sIPSC_LF_subset  %>% filter(Experiment == 'CXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% summarize(mean(median_Amplitude)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_sIPSC_LF_subset %>% filter(Experiment == 'CXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% dplyr::summarize(x = sd(median_Amplitude), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` pA (`r CeA_sIPSC_LF_stats_amp_5nM`).
This chemokine also lowered the sIPSC frequency from `r round(CeA_sIPSC_LF_subset %>% filter(Experiment == 'CXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% summarize(mean(median_Frequency)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_sIPSC_LF_subset %>% filter(Experiment == 'CXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% dplyr::summarize(x = sd(median_Frequency), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` Hz to `r round(CeA_sIPSC_LF_subset  %>% filter(Experiment == 'CXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% summarize(mean(median_Frequency)), digits = 2)` `r knitr::asis_output("\U00B1")` `r round(CeA_sIPSC_LF_subset %>% filter(Experiment == 'CXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% dplyr::summarize(x = sd(median_Frequency), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)` Hz (n = `r CeA_sIPSC_LF_subset %>% filter(Experiment == 'CXCL12(5nM)') %>% ungroup() %>% dplyr::select(Cell) %>% n_distinct()`, N = `r CeA_sIPSC_LF_subset %>% filter(Experiment == 'CXCL12(5nM)') %>% group_by(Filename) %>% mutate(animals = str_sub(Filename, start = 1, end = 5)) %>% ungroup() %>% dplyr::select(animals) %>% n_distinct()`, `r CeA_sIPSC_LF_stats_freq_5nM`, Figs. \@ref(fig:CXCL12CeAsIPSCLF)a3 and b3).
...
CXCL12 was ineffective to modulate both the mIPSCs frequency (`r round(round(CeA_sIPSC_LF_subset %>% filter(Experiment == 'TTX + \nCXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% summarize(mean(median_Frequency)), digits = 2)/round(CeA_sIPSC_LF_subset %>% filter(Experiment == 'TTX + \nCXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% summarize(mean(median_Frequency)), digits = 2), digits = 2)` `r knitr::asis_output("\U00B1")``r round((round(CeA_sIPSC_LF_subset %>% filter(Experiment == 'TTX + \nCXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% dplyr::summarize(x = sd(median_Frequency), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)) / (round(CeA_sIPSC_LF_subset %>% filter(Experiment == 'TTX + \nCXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% dplyr::summarize(x = sd(median_Frequency), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)), digits = 2)`% of baseline, `r CeA_sIPSC_LF_stats_freq_ttx5nM`) 
and amplitude (`r round(round(CeA_sIPSC_LF_subset %>% filter(Experiment == 'TTX + \nCXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% summarize(mean(median_Amplitude)), digits = 2)/round(CeA_sIPSC_LF_subset %>% filter(Experiment == 'TTX + \nCXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% summarize(mean(median_Amplitude)), digits = 2), digits = 2)` `r knitr::asis_output("\U00B1")``r round((round(CeA_sIPSC_LF_subset %>% filter(Experiment == 'TTX + \nCXCL12(5nM)' & Condition == "CXCL12") %>% ungroup() %>% dplyr::summarize(x = sd(median_Amplitude), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)) / (round(CeA_sIPSC_LF_subset %>% filter(Experiment == 'TTX + \nCXCL12(5nM)' & Condition == "baseline") %>% ungroup() %>% dplyr::summarize(x = sd(median_Amplitude), count = n()) %>% dplyr::summarize(x/sqrt(count)), digits = 3)), digits = 2)`% of baseline, n = `r CeA_sIPSC_LF_subset %>% filter(Experiment == 'TTX + \nCXCL12(5nM)') %>% ungroup() %>% dplyr::select(Cell) %>% n_distinct()`, N = `r CeA_sIPSC_LF_subset %>% filter(Experiment == 'TTX + \nCXCL12(5nM)') %>% group_by(Filename) %>% mutate(animals = str_sub(Filename, start = 1, end = 5)) %>% ungroup() %>% dplyr::select(animals) %>% n_distinct()`, `r CeA_sIPSC_LF_stats_amp_ttx5nM`). 
...
These findings collectively illustrate that the regulation of GABAergic transmission by CXCL12 is cell-specific and dependent on spontaneous neuronal activity and mediated *via* direct activation of CXCR4 and astrocytes.

```{r CXCL12CeAsIPSCLF, fig.cap='**CXCL12 modulates GABAergic transmission onto Late-Firing neurons** **(a1-6)** Schematics of presented conditions. Boxplots presenting population data of sIPSCs frequency **(b1)** and amplitude **(b2)** under different conditions. **(c1-4)** Representative current traces illustrating inhibitory synaptic currents (upward deflections) before (blue) and after (red) CXCL12 application across different conditions in a Late-Firing neuron. Scale bar: 10pA, 1s', fig.align = 'center', echo = FALSE, message= FALSE, echo = FALSE, warning= FALSE, message = FALSE, fig.height= 10, fig.width= 15}

LF_representative_cxcl12_5nM <- fig("images//results//cxcl12_cea_sIPSC_LF_cxcl12.png") # , , aspect.ratio = 'free'
# 
LF_representative_amdcxcl12_5nM <- fig("images//results//cxcl12_cea_sIPSC_LF_amdcxcl12.png") # , aspect.ratio = 'free'
# 
LF_representative_fluorocxcl12_5nM <- fig("images//results//cxcl12_cea_sIPSC_LF_fluorocxcl12.png") # , aspect.ratio = 'free'
# 
LF_representative_ttxcxcl12_5nM <- fig("images//results//cxcl12_cea_sIPSC_LF_ttxcxcl12.png")
...
#using patchwork to do layout of the figure
A <- (inset_cxcl12_2 | inset_cxcl12_5 | inset_amdcxcl12_5 | inset_amd | inset_astro_cxcl12_5 | inset_ttxcxcl12_5) + plot_layout(tag_level = "new")
B <- (bp_CeA_freq_sIPSC_LF / bp_CeA_amp_sIPSC_LF) + plot_layout(tag_level = "new")
#Y <- A / B
C <- (LF_representative_cxcl12_5nM) + ggtitle("CXCL12(5nM)")
D <- (LF_representative_amdcxcl12_5nM) + ggtitle("AMD3100 + \nCXCL12(5nM)")
E <- (LF_representative_fluorocxcl12_5nM) + ggtitle("fluorocitrate + \nCXCL12(5nM)")
G <- (LF_representative_ttxcxcl12_5nM) + ggtitle("TTX + CXCL12(5nM)")
X <- (C / D / E / G) + plot_layout(tag_level = "new")
