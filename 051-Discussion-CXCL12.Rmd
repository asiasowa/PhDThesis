...
Nevertheless, it demonstrated neuronal expression of the CXCR4 receptor as well as the CXCL12-induced CXCR4-dependent enhancement in the excitatory synaptic transmission in the BLA.
...
In contrast, results presented here showed that CXCL12(2nM) enhanced sEPSC frequency in `r pie_BLA_c2_freq$data$count[2]` out of `r (pie_BLA_c2_freq$data$count[1] + pie_BLA_c2_freq$data$count[2] + pie_BLA_c2_freq$data$count[3])` neurons, while reducing this parameter in `r pie_BLA_c2_freq$data$count[1]` out of `r (pie_BLA_c2_freq$data$count[1] + pie_BLA_c2_freq$data$count[2] + pie_BLA_c2_freq$data$count[3])` cells (Fig. \@ref(fig:CXCL12BLAsEPSCfreq)a and b1).
Thus, grouped data showed no statistically significant change in the frequency of sEPSCs.
These neuronal proportions were also similar when a higher concentration of exogenous CXCL12, i.e., 5nM, was applied (Fig.\@ref(fig:CXCL12BLAsEPSCfreq)a and b2).
...
Therefore, these findings raise the possibility that the enhancement in excitatory synaptic transmission onto principal cells in the BLA seen in @yang_2016 study, was distorted by the modulation of GABAergic heterogeneous group of local interneurons.
Given the well-known tight regulation of principal cell activity by local interneurons<!--citations-->, along with documented CXCL12 mediation of GABAergic transmission<!--citations-->, this explanation is very likely.
<!-- jeszcze coś o excitability?-->
Nevertheless, whether blocking GABA~A~ receptors will result in a significant increase of sEPSC frequency onto principal cells in the BLA and/or enhancing their firing in rat coronal acute slices remains to be confirmed.
...
This metabolite was reported to exert its action indifferent to the CXCR4 receptor [@zhang_2003].<!--Indeed, it has been shown that CXCL12 can be cleaved by matrix metalloproteases (MMP) present in the brain to give rise to a neurotoxic fragment, namely CXCL12 a (5–67), that exerts its effect through a G-protein coupled receptor distinct from CXCR4 (Zhang et al ., 2003). sprawdzić skąd to... -->
...
Given that upregulated GABAergic transmission in the BLA is often reported in animal models of neuropsychiatric diseases, further experiments using specific MPP inhibitors should clarify this point [@guyon_2005].<!--zmienić-->
...
Notably, given that no CXCR4-expressing astrocytes were observed in the BLA [@yang_2016], this hypothesis could apply more for microglia.
Consistently, CXCR4 is also expressed by microglial cells [@tanabe_1997], the possibility that the concentration needed to activate CXCR4 on microglial might be different from neuronal CXCR4.
...
Many reports show the ACKR3-modulated action of CXCL12 in many brain structures [@fumagalli_2020; @meyrath_2020; @puchert_2017].
Nevertheless, to date, no ACKR3 has been detected in the amygdala [@schnemeier_2008], suggesting that this explanation is rather not plausible.


Altogether, the subtle CXCL12-modulated changes on synaptic excitability and GABAergic transmission might be related with heterogenous groups of principal cells and/or interneurons.
Nevertheless, this study raises the possibility that CXCL12 might modulate the inhibitory synaptic transmission in the BLA in an opposing manner depending on the concentration used.
This is similar to what was observed in other structures and raises the possibility that this chemokine might exert either a neuroprotective or neurotoxic role based on its concentration. 
...
This dissertation hereby provides new evidence for the neuromodulatory properties of the CXCL12/CXCR4 axis in synaptic transmission and membrane properties in the lateral subdivision of the central complex of the amygdala.
...
Next, a cell-specific trend illustrated the opposing CXCL12-mediated modulation of glutamatergic synaptic transmission based on the concentration, precisely onto Regular-Spiking neurons.
...
Moreover, the pre-incubation slices with astrocytic toxin, fluorocitrate, did not abolish the CXCL12-triggered increase in neuronal excitability, suggesting that astrocytes did not mediate this effect.
...
Regarding excitatory synaptic transmission, a similar opposing trend based on concentration as in the BLA was observed. 
At 2nM, CXCL12 reduced the frequency of sEPSC, whereas the elevated frequency of sEPSC was observed during 5nM CXCL12 treatment..
...
The effect of CXCL12 on GABAergic synaptic transmission is complex.
...
The tonic activity of CXCR4 seems not to affect GABAergic transmission. 
Astrocytes modulate this effect.
...
Moreover, the pharmacological inhibition of either microglia or astrocytes is also a caveat, as it might not be specific. 
The optogenetic technique allows for precise temporal(milisecond) and spatial(specific cells types) light activation, thereby modulating cellular excitability, highlighting specificity and bidirectionally.<!-- Light activation of these proteins modulates cellular excitability with millisecond precision. //  Comparison of electrical and optogenetic stimulation paradigms, highlighting cellular specificity and bidirectionality of optogenetic techniques-->

The presented results reveal that both chemokines mainly modulated the the amygdala's neuronal firing and/or GABAergic transmission. 
It should be noted that during whole-cell patch-clamp recording, the cytoplasm of recorded neurons is dialyzing with intrapipette solution. 
...
Finally, GABA~C~ receptor has also been implicated in the regulation of anxiety in the amygdala and its action should be isolated in experiments [@cunha_2010]. 
