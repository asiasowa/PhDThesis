...
% \usepackage[font=doublespacing]{caption} I guess it is already there
\usepackage{setspace}
\usepackage{titlesec}
\usepackage{xcolor}% \usepackage[table]{xcolor}
\usepackage{float}
\usepackage{colortbl} % allows coloring of individual cells
\usepackage{natbib} %- maybe different bibliography, we will see if there will be mistake in the future
...
%\usepackage[tocgraduated]{tocbasic} % This package is still in alpha, but we need it for VU {tocstyle}
\usepackage{array}
...
%\usepackage[Bjornstrup]{fncychap} %Lenny też dawał radę Bjornstrup
